...
class FSGNN(nn.Module):
    def __init__(self,nfeat,nlayers,nhidden,nclass,dropout):
        ...
        self.fc0=nn.Linear((nfeat+nclass),int(nhidden))
        self.fc1 = nn.ModuleList([nn.Linear((nhidden+nclass),int(nhidden)) for _ in range(nlayers-1)])
        self.att = nn.Parameter(torch.ones(nlayers))
        self.sm = nn.Softmax(dim=0)
        #self.sm_alpha = nn.Softmax(dim=1)

        self.alphas=nn.Parameter(torch.ones(nlayers,2))
    ...
    def forward(self,adj, features,y, layer_norm):
        ...
        for i in range(self.nlayers):
            ...
            y_all=torch.mul(mask_alpha[1],y_all)
            #print("y_all:::",y_all)
            '''list_feat=list()
