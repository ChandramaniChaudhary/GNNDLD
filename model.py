...
class FSGNN(nn.Module):
    ...
    def forward(self,adj, features,y, layer_norm):
        ...
        for i in range(self.nlayers):
            ...
            features=  tmp_out 
            wtd_out=torch.mul(mask[i],tmp_out)   
            layer_list.append(wtd_out)
    ...
    '''def forward(self,adj, features,y, layer_norm):
        ...
        mask = self.sm(self.att)
        pow_adj=adj
        Z_init=self.fc_features(features)
        for i in range(self.nlayers):
            mask_alpha=self.sm(self.alphas[i])
            ...
            y_all=torch.add(y_in.to_dense(),y_out.to_dense())#.to_sparse()#.to(device)
            #for ind in range(4):
            Z_init=torch.mul(mask_alpha[0],Z_init)
            hop_features=torch.mul(mask_alpha[1],hop_features)
            init_hop_feat=torch.cat((Z_init,hop_features),dim=1)
            y_out=torch.mul(mask_alpha[2],y_out)
            y_in=torch.mul(mask_alpha[3],y_in)
            y_all=torch.mul(mask_alpha[4],y_all)
            final_feat=torch.cat((torch.cat((torch.cat((init_hop_feat,y_out),dim=1),y_in),dim=1), y_all.to_dense()),dim=1)
            mat=final_feat#torch.cat(final_feat, dim=1)#.to(device)
